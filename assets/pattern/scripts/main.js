...
let sketch = function(p) {
	...
	p.draw = function() {
		...
		for (let i = 0; i < 9; i++) {
			...
			if (parseInt(coord_path.length) >= 2) {
				for (let j = 0; j < coord_path.length - 1; j++) {
					p.line(
						nodes[coord_path[j]].x,
						nodes[coord_path[j]].y,
						nodes[coord_path[j + 1]].x,
						nodes[coord_path[j + 1]].y
					);
				}
			}
		}
		if (parseInt(coord_path.length) >= 1 && parseInt(coord_path.length) < 10) {
			p.line(
				nodes[coord_path[coord_path.length - 1]].x,
				nodes[coord_path[coord_path.length - 1]].y,
				p.mouseX,
				p.mouseY
			);
		}
