<script lang="ts">
    ...
    interface Props {
        blogs: CollectionEntry<"notes">[];
    }

    const { blogs = [] }: Props = $props();

    let tags = $state(
        Array.from(new Set(blogs.flatMap((b) => b.data.tags).map((b) => b!.id))).map(
            (d) => [d, false as boolean] as const
        )
    );

    let activeTags = $derived(tags.filter((t) => t[1]).map((t) => t[0]));
</script>

<Tags bind:tags />

<div class="flex flex-col gap-3 pb-5 md:grid md:grid-cols-3 md:gap-2">
    {#each blogs as blog}
        {#if activeTags.length == 0 || getBlogTags(blog).filter( (v) => activeTags.includes(v) ).length > 0}
            <a
                href={`/notes/${blog.id.split("/").slice(3).join("/")}`}
